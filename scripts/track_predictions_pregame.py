...
def _season_reversion():
    """From config, so it cannot drift from the engine CLI or the harnesses."""
    try:
        return float(load_yaml(get_config_path("settings.yaml"))["elo"]["season_reversion"])
    except Exception as e:
        log(f"[WARN] season_reversion fell back to 0.75 ({e})")
        return 0.75
...
def build_engine(games):
    ...
    engine = TeamELOEngine(
        ...
        season_reversion=_season_reversion())
